  ...
  return bootstrap.initialize(app)   // set up various config files, establish database, etc
    ...
    .then(({app, secretStrings})=>{
      return new Promise((resolve, reject)=>{
        ...
        // set environment variables HOST and PORT appropriately for sites hosted on the same server
        const defaultPort = 2000;
        const defaultHost = '127.0.0.1';
        app.set('port', process.env.PORT || defaultPort);
        app.set('host', process.env.HOST || defaultHost);
        ...
        const csrfExcludedRoutes = [
          '/handy-ncsrf',
          // skip csrf checking for follow routes to pass shopify moderation (they tested app in incognito mode, where 3-party cookie disallowed)
          // todo: implement another way request verification!
          '/handy/shopify/admin/plan',
          '/handy/shopify/admin/canceladminnotification',
          '/handy/shopify/embed/speed_version',
          '/handy/shopify/admin/settings',
          '/handy/shopify/admin/update_order',
        ];
